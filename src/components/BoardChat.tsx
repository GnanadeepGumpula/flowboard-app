...
interface BoardChatProps {
  ...
  members: any[]; // Using the same member structure from your board
}
...
export default function BoardChat({ boardId, currentUserId, members }: BoardChatProps) {
  ...
  const handleDelete = async (msgId: string) => {
    // Soft delete to keep reply threads intact
    const { error } = await supabase
      .from("board_messages")
  ...
  return (
    <>
      ...
      <Button
        ...
        className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-2xl transition-transform hover:scale-105 z-50"
      >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 flex h-[500px] w-80 sm:w-96 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl transition-all">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
            {messages.length === 0 ? (
              <p className="text-center text-sm text-slate-500 mt-10">No messages yet. Say hi!</p>
            ) : (
              messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                    ...
                    <div className={`group relative max-w-[85%] rounded-2xl px-4 py-2 text-sm shadow-sm ${
                      ...
                      {!msg.is_deleted && replyMsg && (
                        <div className={`mb-1 border-l-2 pl-2 text-xs opacity-75 ${isMe ? "border-indigo-300" : "border-slate-300"}`}>
                          <span className="font-semibold">{replyMsg.user_id === currentUserId ? "You" : getMemberProfile(replyMsg.user_id)?.full_name || "User"}: </span>
                      ...
                      {/* Hover Actions */}
                      {!msg.is_deleted && (
                        <div className={`absolute -top-3 hidden items-center gap-1 rounded-md border bg-white p-1 shadow-sm group-hover:flex ${isMe ? "right-0" : "left-0"}`}>
                      ...
                      <div className={`mt-1 flex flex-wrap gap-1 ${isMe ? "justify-end" : "justify-start"}`}>
                        {Object.entries(msg.reactions).map(([emoji, users]) => (
                          <button 
                            ...
                            className={`flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px] bg-white ${users.includes(currentUserId!) ? "border-indigo-300 bg-indigo-50" : "border-slate-200"}`}
                          >
                            <span>{emoji}</span>
          ...
          <div className="border-t bg-white p-3">
            ...
            {(replyingTo || editingMsg) && (
              <div className="mb-2 flex items-center justify-between rounded-md bg-slate-100 px-3 py-1.5 text-xs text-slate-600">
                <div className="line-clamp-1 flex-1">
                  <span className="font-semibold text-indigo-600">{editingMsg ? "Editing message" : `Replying to ${getMemberProfile(replyingTo!.user_id)?.full_name || "User"}`}</span>
                  <span className="ml-2">{editingMsg ? editingMsg.content : replyingTo?.content}</span>
                </div>
            ...
            <form onSubmit={handleSend} className="flex items-center gap-2">
              ...
              />
              <Button type="submit" disabled={!inputValue.trim()} size="sm" className="h-10 w-10 rounded-full bg-indigo-600 hover:bg-indigo-700">
